...
class ServiceSpeech {
    ...
    fun speakByteStream(baos: ByteArrayOutputStream) {
        initAudioTrack().let {
            audioTrack = it.audioTrack.apply {
                ...
                writeBytes(bufferSize = it.bufferSize, track = this, baos = baos)
            }
            audioTrack?.apply {
                stop()
                flush()
    ...
    private fun setupVoiceId(voiceId: String?): Unit = when (voiceId) {
        null -> this.voiceId = Constants.TTS_VOICE_ID
    ...
    private fun getConfiguration(): TTSClientConfiguration = TTSClientConfiguration().apply {
        ...
        port = TTS_PORT
        language = AZ_LANG
        ttsId = if (voiceId.isEmpty()) Constants.TTS_VOICE_ID else voiceId
        isSSLEnabled = true
    }
